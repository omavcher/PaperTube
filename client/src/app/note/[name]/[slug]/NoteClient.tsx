...
import {
  Download, ThumbsUp, Share2, Eye, MessageSquare, Home, ArrowLeft,
  Youtube, UserCheck, Loader2, Trash2, ChevronDown, ChevronUp, Edit
} from "lucide-react";
...
interface NoteClientProps {
  initialData: PublicNoteData;
  username: string;
  slug: string;
}

export default function NoteClient({ initialData, username, slug }: NoteClientProps) {
  const router = useRouter();
  
  // Initialize with Server Data
  const [data, setData] = useState<PublicNoteData | null>(initialData);
  const [comments, setComments] = useState<Comment[]>([]);
  const [currentUser, setCurrentUser] = useState<UserProfile | null>(null);
  
  // UI States
  // loading is false initially because we have server data
  ...
  // On Mount: Load Comments & User Profile
  useEffect(() => {
    if (initialData) {
        fetchCurrentUser();
        loadComments(initialData._id);
    }
  }, [initialData, fetchCurrentUser, loadComments]);
  ...
  const handleDeleteReply = async (commentId: string, replyId: string) => {
    ...
    setComments(prev => prev.map(c => {
        if(c._id === commentId) return { ...c, replies: c.replies.filter(r => r._id !== replyId) };
        return c;
    }));
      ...
      if (c._id === commentId) {
        return {
          ...
          replies: c.replies.map(r => {
            if (r._id === replyId) return { ...r, userLiked: !r.userLiked, likes: r.userLiked ? r.likes - 1 : r.likes + 1 };
            return r;
          })
        };
      }
  ...
  if (loading || !data) return <LoaderX />;
  ...
  return (
    <div className="h-dvh-screen w-screen bg-black flex flex-col overflow-hidden text-neutral-200 font-sans">
      <LoginDialog isOpen={showLoginDialog} onClose={() => setShowLoginDialog(false)} onSuccess={() => {setShowLoginDialog(false); /* Reload Auth */ fetchCurrentUser(); }} />
          ...
          <div className="p-4 border-b border-neutral-800 bg-neutral-950/50 backdrop-blur shrink-0">
               <div className="space-y-4">
                  <div className="flex items-center justify-between">
                     ...
                     {isOwnNote ? (
                         <Button size="sm" variant="secondary" onClick={() => router.push('/profile')} className="rounded-full px-4 h-9 font-medium bg-neutral-800 text-white hover:bg-neutral-700 transition-all border border-neutral-700">
                         ...
                         <Button 
                           ...
                           className={`rounded-full px-4 h-9 font-medium transition-all ${data.creator.isFollowing ? "bg-neutral-800 text-white hover:bg-neutral-700" : "bg-white text-black hover:bg-neutral-200"}`}
                         >
                           ...
                           <div className="flex-1">
                              ...
                              <div className="flex items-center gap-4 mt-2">
                                 ...
                                 {comment.replies && comment.replies.length > 0 && (
                                    <button onClick={() => toggleReplies(comment._id)} className="flex items-center gap-1 text-xs text-blue-400 hover:text-blue-300 font-medium ml-2 bg-blue-500/10 px-2 py-0.5 rounded-full">
                                       {expandedReplies.has(comment._id) ? <><ChevronUp className="h-3 w-3"/> Close</> : <><ChevronDown className="h-3 w-3"/> {comment.replies.length} replies</>}
                                    </button>
                                 )}
                              </div>
      ...
      <MobileBottomNav activeTab={mobileView} setActiveTab={setMobileView} onHome={() => router.push('/')} commentsCount={data.commentsCount} />
    </div>
  );
}