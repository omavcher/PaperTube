...
export default function SuccessStoriesPage() {
  ...
  useEffect(() => {
    const fetchStories = async () => {
      try {
        setLoading(true);
        const response = await api.get("/general/story/all");
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white selection:bg-red-900/50 pb-20 overflow-x-hidden font-sans">
      ...
      <div className="container mx-auto px-3 md:px-4 py-6 relative z-10 max-w-7xl">
        ...
        <section>
          ...
          {/* GRID CONFIGURATION:
             - grid-cols-2: Ensures 2 cards per row on mobile.
             - gap-3: Tighter gap for mobile screens.
          */}
...
// --- Compact Success Card Component ---
...
const SuccessCard = ({ story, delay }: { story: Story, delay: number }) => (
  ...
  >
    ...
    <div className="w-full aspect-[4/3] md:aspect-video relative bg-neutral-900 overflow-hidden">
      ...
      {/* Overlay Info (Mobile Only) - Saves space vertically */}
    ...
    {/* Content Body (Hidden details on very small screens, visible on tap/hover logic if needed, but here we show summary) */}
    <div className="p-3 md:p-5 flex flex-col flex-1 bg-[#0f0f0f]">
      {/* Desktop Header (Hidden on Mobile to save space since we overlay it) */}
      ...
      <Link href={`/success-stories/${story.slug}`} className="w-full mt-auto">
