...
export default function NotePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Generate PDF entirely in the browser — no backend call needed
  const generatePDF = async () => {
    ...
    try {
      // Dynamic imports — avoids SSR issues in Next.js
      const [{ default: jsPDF }, { default: html2canvas }] = await Promise.all([
        import('jspdf'),
        import('html2canvas'),
      ]);

      // Build a white, print-ready container off-screen
      ...
      titleEl.style.cssText = 'font-size:22px;font-weight:800;color:#0f172a;margin:0 0 8px;border-bottom:3px solid #3b82f6;padding-bottom:12px;';
      titleEl.textContent = data.title || 'Note';
      container.appendChild(titleEl);

      // Meta
      const metaEl = document.createElement('p');
      metaEl.style.cssText = 'font-size:11px;color:#64748b;margin:0 0 28px;font-family:monospace;';
      ...
      styleEl.textContent = `
        .px-pdf-body h1{font-size:19px;font-weight:800;color:#0f172a;margin:22px 0 10px}
        .px-pdf-body h2{font-size:16px;font-weight:700;color:#1d4ed8;margin:18px 0 9px;border-bottom:1px solid #e2e8f0;padding-bottom:4px}
        .px-pdf-body h3{font-size:14px;font-weight:700;color:#0f172a;margin:14px 0 7px}
        .px-pdf-body p{margin:0 0 12px}
        .px-pdf-body ul,.px-pdf-body ol{padding-left:22px;margin:0 0 12px}
        .px-pdf-body li{margin-bottom:4px}
        .px-pdf-body strong{font-weight:700}
        .px-pdf-body em{font-style:italic}
        .px-pdf-body a{color:#3b82f6;text-decoration:none}
        .px-pdf-body code{background:#f1f5f9;color:#0f172a;padding:1px 5px;border-radius:3px;font-size:12px;font-family:monospace}
        .px-pdf-body pre{background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px;padding:12px;margin:12px 0;overflow:hidden}
        .px-pdf-body pre code{background:none;padding:0}
        .px-pdf-body blockquote{border-left:4px solid #3b82f6;padding-left:12px;color:#475569;font-style:italic;margin:12px 0}
        .px-pdf-body table{width:100%;border-collapse:collapse;margin:12px 0;font-size:12px}
        .px-pdf-body th{background:#f8fafc;font-weight:600;text-align:left;padding:8px 10px;border:1px solid #e2e8f0}
        .px-pdf-body td{padding:7px 10px;border:1px solid #e2e8f0}
        .px-pdf-body img{max-width:100%;height:auto;border-radius:6px;margin:8px 0;display:block}
      `;
      container.appendChild(styleEl);

      // Content body
      const bodyEl = document.createElement('div');
      bodyEl.className = 'px-pdf-body';
      bodyEl.innerHTML = markdownContent;
      container.appendChild(bodyEl);

      document.body.appendChild(container);

      // Capture at 2× for sharp text
      ...
      // A4 in points (72 dpi): 595.28 × 841.89 pt
      ...
      const printH = pageH - margin * 2;   // max content height per page in pts
      const sliceH = Math.floor(printH / ratio); // canvas px per page
