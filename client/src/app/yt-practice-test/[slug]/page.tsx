...
export default function PracticeTestPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      const token = localStorage.getItem('authToken');
      const res = await api.get(`/test/${slug}/export`, {
        headers: { 'Auth': token }
      });
      if (res.data?.success && res.data?.data?.pdf) {
        ...
        a.href = url;
        a.download = res.data.data.fileName || `${testData?.title || 'Practice_Test'}.pdf`;
  ...
  const answeredCount = QUESTIONS.filter((mockQ: any) => answers[mockQ.id] !== undefined && answers[mockQ.id] !== '' && (!Array.isArray(answers[mockQ.id]) || answers[mockQ.id].length > 0)).length;
  const answeredPercent = QUESTIONS.length ? (answeredCount / QUESTIONS.length) * 100 : 0;
           ...
           <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl w-full relative z-10 text-center">
              ...
              <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
                 ...
                 <button 
                    ...
                    className="inline-flex items-center justify-center gap-3 px-8 py-4 bg-[#0a0a0a] border border-white/10 hover:border-white/20 text-white rounded-2xl font-bold uppercase tracking-widest text-xs hover:bg-neutral-900 transition-all active:scale-95"
                 >
                    {isExporting ? <Loader2 className="animate-spin" size={14} /> : "Export Practice Sheet"}
                 </button>
              </div>
              <div className="mt-6">
        ...
        <div className="min-h-[100dvh] bg-[#060606] text-white font-sans pb-32">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-xl mx-auto pt-40 px-4 mb-20 text-center">
               ...
               <div className="flex justify-center gap-3">
                  <button onClick={() => { setStarted(false); setSubmitted(false); setAnswers({}); setFlaggedQuestions({}); setCurrentQIndex(0); setTimeLeft(QUESTIONS.length * 60); }} className="px-6 py-3 bg-white/[0.05] border border-white/10 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-white/10 transition-colors flex justify-center items-center gap-2">
                      <RefreshCw size={14} /> Retry Test
                  </button>
                  <button 
                      onClick={handleExportPDF}
                      disabled={isExporting}
                      className="px-6 py-3 bg-white text-black rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-neutral-200 transition-colors flex justify-center items-center gap-2"
                  >
                      {isExporting ? <Loader2 className="animate-spin" size={14} /> : "Export Test (PDF)"}
                  </button>
               </div>
          </motion.div>
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          flaggedQuestions[q.id]
                            ? "bg-amber-500/10 border-amber-500/40 text-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.15)]"
                            : "bg-white/5 border-white/10 text-neutral-400 hover:text-white hover:border-white/20"
                        )}
                      >
                        <Flag size={11} className={flaggedQuestions[q.id] ? "fill-amber-500" : ""} />
             ...
             <div className="bg-[#0a0a0a] border border-white/5 rounded-2xl p-6 sticky top-[80px]">
                ...
                <div className="grid grid-cols-5 gap-2">
                   {QUESTIONS.map((mockQ: any, i: number) => {
                      ...
                      const isFlagged = flaggedQuestions[mockQ.id];
                      return (
                          <button 
                              key={mockQ.id || i}
                              onClick={() => setCurrentQIndex(i)}
                              className={cn(
                                  "aspect-square rounded-xl font-bold text-xs flex items-center justify-center transition-all bg-[#0a0a0a] relative",
                                  currentQIndex === i ? "border-[2px] border-white text-white shadow-[0_0_15px_rgba(255,255,255,0.2)] scale-110" :
                                  isFlagged ? "border-2 border-amber-500 text-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.2)]" :
                ...
                <div className="mt-8 pt-6 border-t border-white/5 space-y-3">
                   ...
                   <div className="flex items-center gap-3 text-[11px] font-bold uppercase tracking-widest text-neutral-400">
                      <div className="w-4 h-4 rounded border border-amber-500/30 bg-amber-500/10 shrink-0"></div> Flagged
                   </div>
                   <div className="flex items-center gap-3 text-[11px] font-bold uppercase tracking-widest text-neutral-400">
                ...
                {currentQIndex === QUESTIONS.length - 1 && (
                  <div className="mt-8 pt-6 border-t border-white/5">
                      <button onClick={() => setShowSubmitModal(true)} className="w-full flex items-center justify-center gap-2 px-6 py-4 bg-white text-black rounded-xl font-black uppercase tracking-widest text-[11px] hover:bg-neutral-200 transition-all shadow-[0_0_30px_rgba(255,255,255,0.15)] active:scale-95">
            ...
            >
               ...
               <div className="flex-1 overflow-y-auto p-6 pb-safe">
                     ...
                     {QUESTIONS.map((mockQ: any, i: number) => {
                        ...
                        return (
                            <button 
                                ...
                                className={cn(
                                    "aspect-square rounded-2xl font-bold text-lg flex items-center justify-center transition-all bg-[#0a0a0a] relative",
                  ...
                  <div className="mt-10 pt-8 border-t border-white/10 space-y-4">
                     ...
                     <div className="flex items-center gap-4 text-xs font-bold uppercase tracking-widest text-neutral-400">
                        <div className="w-5 h-5 rounded flex-shrink-0 border border-amber-500/30 bg-amber-500/10"></div> Flagged
                     </div>
                     <div className="flex items-center gap-4 text-xs font-bold uppercase tracking-widest text-neutral-400">
  ...
  return (
    <div className="min-h-[100dvh] bg-[#060606] text-white flex flex-col font-sans selection:bg-blue-500/30 selection:text-white pb-32 lg:pb-0">
      <div className="flex flex-col min-h-screen w-full">
        {/* Top answered progress bar during test taking */}
        {started && !submitted && (
          <div className="fixed top-16 inset-x-0 h-1 bg-neutral-900 z-50">
            <div 
        ...
        {showSubmitModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-md bg-neutral-900 border border-white/10 rounded-[2rem] p-6 sm:p-8 shadow-2xl z-10 text-center"
            >
              <button onClick={() => setShowSubmitModal(false)} className="absolute top-6 right-6 text-neutral-500 hover:text-white transition-colors">
                <X size={18} />
              </button>
              
              <div className="w-12 h-12 bg-blue-500/10 rounded-2xl border border-blue-500/20 flex items-center justify-center mx-auto mb-4">
                <GraduationCap className="text-blue-500" size={24} />
              </div>
              
              <h3 className="text-xl font-bold text-white mb-2 tracking-tight">Finish Practice Test?</h3>
              <p className="text-xs text-neutral-400 mb-6 leading-relaxed">
                Review your test summary below before submitting for scoring.
              </p>

              <div className="grid grid-cols-3 gap-3 mb-6">
                <div className="bg-black/40 border border-white/5 rounded-xl p-3">
                  <p className="text-[9px] font-bold text-neutral-500 uppercase tracking-wider mb-1">Answered</p>
                  <p className="text-lg font-bold text-green-400">{answeredCount}</p>
                </div>
                <div className="bg-black/40 border border-white/5 rounded-xl p-3">
                  <p className="text-[9px] font-bold text-neutral-500 uppercase tracking-wider mb-1">Unanswered</p>
                  <p className="text-lg font-bold text-red-400">{QUESTIONS.length - answeredCount}</p>
                </div>
                <div className="bg-black/40 border border-white/5 rounded-xl p-3">
                  <p className="text-[9px] font-bold text-neutral-500 uppercase tracking-wider mb-1">Flagged</p>
                  <p className="text-lg font-bold text-amber-400">{Object.values(flaggedQuestions).filter(Boolean).length}</p>
                </div>
              </div>

              {(QUESTIONS.length - answeredCount) > 0 && (
                <div className="mb-6 p-4 rounded-xl bg-red-500/5 border border-red-500/10 text-left flex items-start gap-2.5">
                  <Flag size={14} className="text-red-500 mt-0.5 shrink-0 animate-pulse" />
                  <p className="text-[11px] text-red-400 leading-normal font-medium">
                     You still have {QUESTIONS.length - answeredCount} unanswered question(s). Submitting now will score them as incorrect.
                  </p>
                </div>
              )}
              ...
              <div className="flex gap-3">
                <button 
                  onClick={() => setShowSubmitModal(false)}
                  className="flex-1 h-11 bg-neutral-800 hover:bg-neutral-700 text-neutral-300 hover:text-white rounded-xl font-bold uppercase tracking-widest text-[10px] border border-white/5 transition-all"
                >
                  Back to Test
                </button>
                <button 
                  ...
                  className="flex-1 h-11 bg-white text-black hover:bg-neutral-200 rounded-xl font-bold uppercase tracking-widest text-[10px] transition-all shadow-md animate-shimmer"
                >
                  Submit Test
                </button>
              </div>
