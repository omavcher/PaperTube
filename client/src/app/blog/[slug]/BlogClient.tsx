...
import { 
  ArrowLeft, Share2, Menu, X, ChevronRight, 
...
export default function BlogClient({ post }: { post: BlogPost }) {
  ...
  // Analytics & TOC Gen
  useEffect(() => {
    // 1. Generate TOC if missing from DB
    if (post) {
    ...
    const trackView = async () => {
        try {
            ...
            console.error("Tracking Error (Non-critical)", e);
        }
    };
    trackView();
  ...
  const handleShare = async () => {
    if (navigator.share) {
      try {
        ...
        api.post('/general/track', { type: 'blog', id: post.slug, metric: 'share' });
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-red-900/50 pb-24 md:pb-20">
      ...
      {/* Mobile Table of Contents (Slide-over) */}
      <AnimatePresence>
        {mobileMenuOpen && (
          <>
            <motion.div 
