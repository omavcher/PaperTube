...
export default function BlogResourcesPage() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        setLoading(true);
        const response = await api.get("/general/blog/all");
  ...
  const totalPages = Math.ceil(listedPosts.length / itemsPerPage);
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white selection:bg-red-900/50  font-sans overflow-x-hidden">
      ...
      <div className="container mx-auto px-3 md:px-4 py-4 md:py-8 relative z-10 max-w-7xl">
          ...
          <div className="relative w-full md:w-80 z-50" ref={searchRef}>
            <div className="relative">
              ...
              <Input 
                ...
                className="bg-[#111] border-white/10 rounded-xl pl-10 pr-10 h-10 md:h-12 text-xs font-bold focus:border-red-600 transition-all placeholder:text-neutral-600"
              />
              {searchQuery && (
                <button 
            ...
            <AnimatePresence>
              {isSearchOpen && (
                <motion.div 
                  ...
                  exit={{ opacity: 0, y: 10 }}
                  className="absolute top-full left-0 right-0 mt-2 bg-[#0f0f0f] border border-white/10 rounded-xl shadow-2xl overflow-hidden max-h-[60vh] overflow-y-auto custom-scrollbar"
                >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12">
          ...
          <aside className="lg:col-span-4 space-y-6 order-2 lg:order-1">
            ...
            {/* Trending Activity */}
            <div className="bg-[#0f0f0f] border border-white/10 rounded-2xl p-4 md:p-5 shadow-lg hidden md:block">
              <h3 className="text-xs md:text-sm font-black uppercase tracking-widest text-neutral-500 mb-4 flex items-center gap-2">
                <TrendingUp size={14} /> Community Pulse
              </h3>
              <div className="space-y-4">
                {POPULAR_POSTS.map((post) => (
                  <div key={post.id} className="flex items-center gap-3 p-3 rounded-xl bg-[#151515] border border-white/5 hover:border-red-600/30 transition-all cursor-pointer group">
                    <div className="relative">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[9px] md:text-[10px] text-neutral-400 truncate">
                        <span className="font-bold text-white">@{post.user}</span> {post.action}
                      </p>
                      <p className="text-[10px] md:text-xs font-bold text-white group-hover:text-red-500 transition-colors truncate">
                        {post.target}
                      </p>
                    </div>
                    <span className="text-[8px] md:text-[9px] font-bold text-neutral-600 whitespace-nowrap">{post.time}</span>
                  </div>
                ))}
              </div>
            </div>
