...
// --- TYPES ---
// Matches the keys in ASSETS.badges
...
interface LeaderboardUser {
  rank: number;
  name: string;
  username: string; // Added to display unique ID if needed
  avatarUrl: string;
  totalXP: number;
  badges: BadgeItem[];
...
// The shape of the API response item
interface ApiUserResponse {
  ...
  avatar: string;
  xp: number;
  badge: string; // "legendary" | "basic" | etc
}
...
export default function GlobalLeaderboardPage() {
  ...
  // Converts API raw object -> UI Component object
  const mapApiUserToUi = (apiUser: ApiUserResponse, currentUserId: string): LeaderboardUser => {
    // Map the single string badge from API to an array of BadgeItems for the UI
    // Ensure the badge string matches a key in ASSETS.badges (fallback to basic if unknown)
    ...
    const badgeItem: BadgeItem = {
      ...
      tier: tier,
      name: apiUser.badge.charAt(0).toUpperCase() + apiUser.badge.slice(1) // Capitalize "legendary" -> "Legendary"
    };
    ...
    return {
      ...
      totalXP: apiUser.xp,
      badges: [badgeItem], // Wrapping single badge in array as UI expects multiple
      ...
      try {
        ...
        try {
          ...
          if (userRaw) {
            ...
            storedUserId = user?._id; // Keep raw ID for comparison
          }
        } catch (err) {
        ...
        // 2. Fetch Data
        const res = await api.post("/users/get-groble-leaderboard", { userId });
        ...
        if (res.data && res.data.success) {
          ...
          // 3. Transform Data
          const formattedLeaderboard = rawLeaderboard.map(user => mapApiUserToUi(user, storedUserId));
  ...
  // Podium: Top 3 users. We need to safeguard if fewer than 3 users exist.
  const podiumOrder = useMemo(() => {
    if (leaderboardData.length === 0) return [];
    
    // UI expects: [2nd Place, 1st Place, 3rd Place]
  ...
  // List: Users from rank 4 onwards
  const listUsers = useMemo(() => {
    return leaderboardData.slice(3); 
  }, [leaderboardData]);

  // Show sticky footer if current user exists AND is not in the top 10 (or whatever logic you prefer)
  // Based on your previous logic, we hide it if they are already visible on screen (e.g. top 10).
  const showStickyFooter = currentUser && currentUser.rank > 10;
          ...
          <>
            ...
            {podiumOrder.length > 0 && (
              <div className="flex justify-center items-end gap-3 md:gap-8 mb-16 h-[340px] md:h-[420px]">
                {podiumOrder.map((user) => {
                   // Calculate position for styling based on rank
                   // If user is rank 1, position=1. If rank 2, position=2.
                   return <PodiumItem key={user.rank} user={user} position={user.rank} />;
                })}
              </div>
            )}
            ...
            >
               ...
               <div className="divide-y divide-white/5">
                  ...
                  {listUsers.length === 0 && leaderboardData.length > 3 && (
                        ...
                        {user.badges.slice(0, 3).map((b) => (
                            <div key={b.id} className="relative group/badge">
                                <img 
                                    ...
                                    className="w-6 h-6 md:w-8 md:h-8 object-contain drop-shadow-[0_0_8px_rgba(255,255,255,0.3)] hover:scale-125 transition-transform" 
                                    title={b.name} 
                                />
                            </div>
                        ))}
        ...
        >
            ...
            <div className="hidden md:flex col-span-2 justify-center items-center gap-2">
                {user.badges.map((badge) => (
                    <div 
                        key={badge.id} 
                        className="relative p-1 rounded-full bg-white/5 border border-white/5 hover:bg-white/10 transition-colors"
                        title={badge.name}
                    >
                        <img 
                            src={ASSETS.badges[badge.tier]} 
                            alt={badge.name}
                            className="w-8 h-8 md:w-9 md:h-9 object-contain drop-shadow-md hover:scale-125 transition-transform cursor-help"
                        />
                    </div>
                ))}
            </div>
                                 ...
                                 <div className="flex gap-2 mt-1">
                                    {user.badges.map(b => (
                                        <img 
                                            key={b.id} 
                                            src={ASSETS.badges[b.tier]} 
                                            className="w-5 h-5 md:w-6 md:h-6 object-contain drop-shadow-sm hover:scale-125 transition-transform" 
                                            title={b.name}
                                        />
                                    ))}
                                 </div>
