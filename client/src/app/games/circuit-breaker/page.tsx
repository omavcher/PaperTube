...
export default function CircuitBreakerGame() {
  ...
  const handleWrong = () => {
    if (lives > 1) {
      setLives(l => l - 1);
      toast.error("Wrong Answer!", { description: "Try a different gate." });
      setSelectedGate(null);
    } else {
  ...
  return (
    <div className="min-h-[100dvh] pt-24 lg:pt-32 relative z-[20] bg-transparent text-white flex flex-col items-center justify-center p-4 md:p-8 selection:bg-blue-500/30 overflow-hidden">
      ...
      <div className="max-w-2xl w-full">
        ...
        <div className="flex justify-between items-center mb-8 bg-neutral-900/20 p-6 rounded-xl md:rounded-2xl border border-white/5 backdrop-blur-md shadow-2xl">
          ...
          <div className="text-right">
            <p className="text-[9px] font-black text-neutral-600 tracking-widest leading-none mb-1">Level {level}</p>
        ...
        <AnimatePresence mode="wait">
          {gameState === 'START' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-10 py-12">
              <div className="space-y-4">
                ...
                <h1 className="text-6xl md:text-8xl font-black tracking-tight  leading-none">Circuit <span className="text-blue-500">Breaker</span></h1>
                <p className="text-neutral-600 text-[10px] md:text-xs font-black tracking-widest">Test your logic gate knowledge</p>
              </div>
          ...
          {gameState === 'PLAYING' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-12">
              {/* Circuit Board */}
              <div className="relative h-64 md:h-80 bg-neutral-900/10 rounded-2xl md:rounded-[5rem] border-2 border-white/5 flex items-center justify-between px-8 md:px-20 overflow-hidden backdrop-blur-sm">
                ...
                <div className="relative z-10">
                  <div className={cn(
                    "h-24 w-24 md:h-32 md:w-32 rounded-[2rem] md:rounded-2xl border-4 border-dashed flex items-center justify-center transition-all duration-500",
                    selectedGate ? "border-blue-500 bg-blue-500/10 scale-110 shadow-lg" : "border-neutral-800"
                  )}>
                    {selectedGate ? (
                      <span className="text-2xl md:text-3xl font-black text-blue-500 ">{selectedGate}</span>
                    ) : (
                ...
                <div className="flex items-center z-10">
                  <div className={cn("h-1 w-12 md:w-20 transition-all duration-1000", isLive ? "bg-blue-500 shadow-md" : "bg-neutral-900")} />
                  <div className={cn(
                    "h-20 w-20 md:h-28 md:w-28 rounded-full flex items-center justify-center transition-all duration-500 border-4",
                    isLive ? "bg-blue-500 border-blue-300 shadow-lg" : "bg-black border-neutral-900"
                  )}>
              ...
              {/* Gate Tray */}
              <div className="space-y-4">
                <p className="text-center text-[9px] font-black text-neutral-700 tracking-widest">Inject_Logic_Module</p>
                <div className="flex flex-wrap justify-center gap-3">
                  {availableGates.map((gate) => (
                    <Button 
                      key={gate}
                      onClick={() => handleGateDrop(gate)}
                      className="h-16 md:h-20 px-8 bg-neutral-900 border border-white/5 hover:border-blue-500 hover:bg-blue-500/10 rounded-2xl text-lg font-black  transition-all active:scale-90"
                    >
                      {gate}
                    </Button>
                  ))}
          ...
          {(gameState === 'GAMEOVER' || gameState === 'SYNCING') && (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-10">
              <div className="text-center bg-neutral-900/50 p-12 rounded-2xl border border-white/10 shadow-2xl backdrop-blur-md relative overflow-hidden">
                ...
                <h2 className="text-6xl font-black text-red-600  tracking-tight mb-12 relative z-10">Game Over</h2>
                ...
                {gameState === 'SYNCING' ? (
                  ...
                  <Button onClick={startGame} className="w-full h-20 bg-white text-black font-black text-2xl rounded-3xl hover:bg-blue-500 hover:text-white transition-all active:scale-95 shadow-2xl">
                    Play Again <RotateCcw size={24} className="ml-3" />
                  </Button>
                )}
              </div>
              ...
              <div className="space-y-4 px-4">
                <div className="flex items-center gap-2 text-neutral-700">
                  <History size={14} />
                  <h3 className="text-[10px] font-black tracking-widest">Match History</h3>
                </div>
                <div className="space-y-3">
                  {history.map((match) => (
                    <div key={match.id} className="bg-neutral-900/20 border border-white/5 p-5 rounded-[2rem] flex justify-between items-center group hover:border-blue-500/20 transition-all">
                      <div className="flex flex-col">
                        <span className="text-[9px] text-neutral-700 font-bold">{match.date}</span>
                        <span className="text-sm font-black text-blue-500 ">Level {match.level} Reached</span>
                      </div>
                      <div className="text-right">
                        <span className="text-[10px] font-black text-neutral-800 block mb-1">Score</span>
                        <span className="text-2xl font-black text-white tracking-tight">{match.score}</span>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <div className="mt-16 flex flex-wrap justify-center gap-12 text-[10px] font-black tracking-widest text-neutral-900">
        ...
        <span className="flex items-center gap-2 font-sans">v1.0.0</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center gap-4">
      <div className={cn(
        ...
        val ? "bg-blue-500 text-black shadow-lg" : "bg-neutral-900 text-neutral-700"
      )}>
        {val}
      </div>
      <div className={cn("h-1 w-12 md:w-24 transition-all duration-500", val ? "bg-blue-500 shadow-md" : "bg-neutral-950")} />
    </div>
  );
}