...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return generateMetadataForPage("ai-study", lowerRegion);
}

export default async function RegionalAIStudyPage({ params }: PageProps) {
  const { region } = await params;
  const lowerRegion = region.toLowerCase();
  ...
  return <AIStudySeoPage region={lowerRegion} />;
}
