...
exports.getUserNotes = async (req, res) => {
  try {
    ...
    if (type === 'premium') {
      query['generationDetails.type'] = 'premium';
    } else if (type === 'free') {
      query['generationDetails.type'] = 'free';
    }
    ...
    const notes = await Note.find(query)
      ...
      .select(
        'title slug thumbnail videoUrl videoId pdf_data views createdAt generationDetails status'
      );

    const totalNotes = await Note.countDocuments(query);
    const totalPremiumNotes = await Note.countDocuments({
      ...query,
    ...
    return res.status(200).json({
      ...
      pagination: {
        ...
        totalPremium: totalPremiumNotes,
        totalFree: totalNotes - totalPremiumNotes
      }
    });
  } catch (error) {
